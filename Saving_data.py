...
class FlightSimulation(gym.Env):
    ...
    def _erf(self, x):
        ...
        #print('CO2 emissions:', kerosene_weight * FLIGHT_CHARACTERISTICS[self.flight_phase]['EI_CO2'])
            ...
            if powerpath[1].name == 'GasTurbine':
                ...
                for phase_indicator, phase in enumerate(self.x):
                    ...
                    try:
                        ...
                        #print('NOX emissions CJF:', kerosene_phase_mass * EI_nox_cor/1000)
                    except:
                        pass
                    try:
                        ...
                        #print('NOX emissions H2:', hydrogen_phase_mass * EI_nox_cor/1000 * 0.76)
                    except:
                        pass
    ...
    def reset(self, pt, powerpaths, A_obj, components):
...
class ActionFeasibilityWrapper(gym.ActionWrapper):
    ...
    def reset(self, pt, powerpaths, matrix, components):
        self.penalties = []
        return self.env.reset(pt, powerpaths, matrix, components)
...
def saving_data(CARGO, FP2050, start_number, end_number, directory, year, n_samples, deterministic, random, saving):
    ...
    for i in range(start_number, end_number+1):
        if i == 2:
            model_name = f"{year}_v{2}"
            model_path = os.path.join(directory, 'Saved Models', model_name)
            model_v2 = SAC.load(model_path, custom_objects={"buffer_size": 1})
            rewards_model_v2 = []
        elif i == 3:
            model_name = f"{year}_v{3}"
            model_path = os.path.join(directory, 'Saved Models', model_name)
            model_v3 = SAC.load(model_path, custom_objects={"buffer_size": 1})
            rewards_model_v3 = []
        elif i == 4:
            model_name = f"{year}_v{4}"
            model_path = os.path.join(directory, 'Saved Models', model_name)
            model_v4 = SAC.load(model_path, custom_objects={"buffer_size": 1})
            rewards_model_v4 = []
        elif i == 5:
            model_name = f"{year}_v{5}"
            model_path = os.path.join(directory, 'Saved Models', model_name)
            model_v5 = SAC.load(model_path, custom_objects={"buffer_size": 1})
            rewards_model_v5 = []
        elif i == 6:
            model_name = f"{year}_v{6}"
            model_path = os.path.join(directory, 'Saved Models', model_name)
            model_v6 = SAC.load(model_path, custom_objects={"buffer_size": 1})
            rewards_model_v6 = []
    ...
    for i in range(0,len(unique_actions)):
        ...
        j = 0
        while not done:
            action = unique_actions[i][j]
            j+=1
            obs, reward, done, info = pt.step(action)
        powerpaths, matrix, components = pt.matrix()

        pt.describe()

        n_cp = len(pt.supply_param) + len(pt.shaft_param)
        architectures.append([i, str(pt.describe_noprint())])

        if random == True:
            # Do many random actions
            rewards_random_loop = []
            for k in range(0,100):
                env.action_space.seed(200*i*k)
        ...
        if 3 >= start_number and end_number >= 3:
            ...
            if reward > highest_reward:
                highest_reward = reward
            print('V3 highest reward:', highest_reward)
            rewards_model_v3.append([np.mean(rewards_v3), energy_sources[i], i, str(info.get("Supply_Parameters"))])

        if 4 >= start_number and end_number >= 4:
            # Use model v4 to predict action
            rewards_v4 = []
            highest_reward = -10
            for k in range(0,n_samples):
                powerpaths, matrix, components = pt.matrix()
    ...
    if 2 >= start_number and end_number >= 2:
        rewards_model_v2 = reorder_by_type(rewards_model_v2)
    if 3 >= start_number and end_number >= 3:
        rewards_model_v3 = reorder_by_type(rewards_model_v3)
    if 4 >= start_number and end_number >= 4:
        rewards_model_v4 = reorder_by_type(rewards_model_v4)
    if 5 >= start_number and end_number >= 5:
        rewards_model_v5 = reorder_by_type(rewards_model_v5)
    if 6 >= start_number and end_number >= 6:
        rewards_model_v6 = reorder_by_type(rewards_model_v6)

    def save_lists(filename, *names, ns=None):
        # auto-detect caller's scope if ns not provided
        if ns is None:
    ...
    if saving == True:
        ...
        if random == True:
            args_to_save.append("rewards_random_avg")
...
def cma(CARGO, FP2050, year):
    ...
    for i in range(0,len(unique_actions)):
        ...
        optimizer = CMA(mean=np.full((len(pt.supply_param) + len(pt.shaft_param))*4, 0.1),
                        bounds=(np.tile([0, 1], ((len(pt.supply_param) + len(pt.shaft_param))*4, 1))),
                        sigma=0.5,
                        n_max_resampling=1)
        ...
        for g in range(generations):
            ...
            rewards = []
            solutions = []
            for k in range(optimizer.population_size):
                # print(k)
                obs = pt.reset()
                done = False
                j = 0
                while not done:
                    # print('Not done for architecture')
                    action = unique_actions[i][j]
                    j += 1
                    obs, reward, done, info = pt.step(action)
                powerpaths, matrix, components = pt.matrix()
                ...
                if reward > highest_reward:
                    highest_reward = reward
                    highest_reward_actions = action
                    highest_reward_arch = pt.describe_noprint()
                if reward > highest_reward_pa:
                    highest_reward_pa = reward
                    highest_reward_actions_pa = action
                    highest_reward_arch_pa = pt.describe_noprint()
                score = evaluate(reward)
